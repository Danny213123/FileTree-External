...
function CountBar({ label, count, total, tone }: { label: string; count: number; total: number; tone?: Tone }) {
  return <div className={`cdl-count-row${tone ? ` cdl-tone-${tone}` : ""}`}><span>{label}</span><progress aria-label={label} max={Math.max(1, total)} value={count} /><span className="cdl-pct">{(total ? count / total * 100 : 0).toFixed(2)}%</span><strong>{count.toLocaleString()}</strong></div>;
}
const fileRows: [string, string, Tone][] = [["completed", "Completed", "green"], ["prev_completed", "Previously downloaded", "yellow"], ["skipped", "Skipped by config", "yellow"], ["queued", "Queued", "cyan"], ["failed", "Failed", "red"]];
const compressionTones: Record<"pending" | "compressed" | "skipped" | "failed", Tone> = { pending: "cyan", compressed: "green", skipped: "yellow", failed: "red" };
...
export function CyberdropSummary({ data, running }: { data?: DashboardData | null; running: boolean }) {
  ...
  return <>
    <div className="cdl-summary-grid">
      <section className="cdl-card"><header><strong>Files</strong><span>Total: {total.toLocaleString()}</span></header><div className="cdl-card-body">{fileRows.map(([key, label, tone]) => <CountBar key={key} label={label} tone={tone} count={stats[key] ?? 0} total={total} />)}</div></section>
      <Errors title="Scrape errors" errors={data?.scrapeErrors?.errors} />
      <Errors title="Download errors" errors={data?.downloadErrors?.errors} />
    </div>
    <section className="cdl-card"><header><strong>Scraping</strong><span>{running ? data?.scraping?.length ?? 0 : 0} active URLs · {(data?.scrapeQueued ?? 0).toLocaleString()} queued</span></header><div className="cdl-scrape-list">{running && data?.scraping?.length ? data.scraping.map((item, index) => <div key={index}><span title={item.url}>{item.url}</span><span className="cdl-eta">{item.elapsed == null ? "" : `${Math.floor(item.elapsed)}s`}</span></div>) : <span className="cdl-muted">{running ? "Waiting for URLs" : "No active scraping"}</span>}</div></section>
...
export function CyberdropCompression({ data, running, mode, bytes, eta }: { data?: DashboardData | null; running: boolean; mode: string; bytes: (value: number) => string; eta: (value: number | null) => string }) {
  ...
  return <section className="cdl-card"><header><strong>{value?.title || "Compression"}</strong><span>Active: {running ? value?.files.length ?? 0 : 0} · Total: {(value?.total ?? 0).toLocaleString()}</span></header>
    ...
    {running && value?.files.map(file => <div className="cdl-compression-file" key={file.id}><strong title={file.name}>{file.name}</strong><progress aria-label={`Compression progress for ${file.name}`} max={file.total || undefined} value={file.total ? file.completed : undefined} /><span className="cdl-pct">{file.total ? `${Math.min(100, file.completed / file.total * 100).toFixed(2)}%` : "Unknown size"}</span><span className="cdl-bytes">{bytes(file.completed)} / {file.total ? bytes(file.total) : "?"}</span><span className="cdl-speed">{file.speed == null ? "Estimating speed" : `${bytes(file.speed)}/s`}</span><span className="cdl-eta">{eta(file.eta)}</span></div>)}
    {(["pending", "compressed", "skipped", "failed"] as const).map(key => <CountBar key={key} label={key[0].toUpperCase() + key.slice(1)} tone={compressionTones[key]} count={value?.[key] ?? 0} total={value?.total ?? 0} />)}
    </div></section>;
}
