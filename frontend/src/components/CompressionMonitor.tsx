...
export function CompressionMonitor({ focusJobId }: Props) {
  ...
  const renderCell = (file: CompressJobFile, key: string) => {
    ...
    switch (key) {
      ...
      case "result": return <span className={`cm-result ${file.status}`} title={file.error}>{file.reason === "skipped_prior_no_gain" ? "Previously no gain" : file.reason === "skipped_already_compressed" ? "Already compressed" : file.reason || file.status}</span>;
      default: return null;
    }
  };
