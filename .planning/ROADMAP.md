...
**Plans:** 4 plans
Plans:
- [ ] 02-01-PLAN.md — Settings module foundation: hand-rolled JSON parse/write + Settings struct + atomic temp+rename store + baseline FFI (Kernel32 MoveFileExW, Shell32 SHGetKnownFolderPath)
- [ ] 02-02-PLAN.md — Single-instance named mutex (`LocalFileTree.SingleInstance.v1`) + WM_COPYDATA path forwarding (memcpy-out discipline, 64 KB cap, path validation)
- [ ] 02-03-PLAN.md — Path bar (ComboBoxEx32 drive picker + EDIT with SHAutoComplete) + 6-shortcut accelerator table (Enter/Esc/Del/Ctrl+F/Ctrl+E/F5)
